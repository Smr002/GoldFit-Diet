...
const FoodSearchModal = ({ open, onClose, onAddFood, mealId }) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.nal.usda.gov/fdc/v1/foods/search?api_key=${API_KEY}&query=${encodeURIComponent(searchQuery)}&pageSize=10`,
        {
          method: 'GET',
          headers: {
  ...
  const handleAddFood = () => {
    ...
    const calories = nutrients.find(n => n.nutrientName === 'Energy' || n.nutrientNumber === '208')?.value || 0;
    const protein = nutrients.find(n => n.nutrientName === 'Protein' || n.nutrientNumber === '203')?.value || 0;
    const carbs = nutrients.find(n => n.nutrientName === 'Carbohydrate, by difference' || n.nutrientNumber === '205')?.value || 0;
    const fat = nutrients.find(n => n.nutrientName === 'Total lipid (fat)' || n.nutrientNumber === '204')?.value || 0;
                      ...
                      <Grid container spacing={1}>
                        <Grid item xs={6}>
                          <Typography variant="body2">
                            Calories: <strong>{Math.round(calories * multiplier)}</strong> kcal
                          </Typography>
                        </Grid>
                        <Grid item xs={6}>
                          <Typography variant="body2">
                            Protein: <strong>{(protein * multiplier).toFixed(1)}</strong> g
                          </Typography>
                        </Grid>
                        <Grid item xs={6}>
                          <Typography variant="body2">
                            Carbs: <strong>{(carbs * multiplier).toFixed(1)}</strong> g
                          </Typography>
                        </Grid>
                        <Grid item xs={6}>
                          <Typography variant="body2">
                            Fat: <strong>{(fat * multiplier).toFixed(1)}</strong> g
                          </Typography>
                        </Grid>
                      </Grid>
                    );
