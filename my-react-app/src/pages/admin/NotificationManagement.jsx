...
const NotificationManagement = () => {
  ...
  const [notifications, setNotifications] = useState([
    {
      id: 1,
      type: 'System',
      message: 'Welcome to GoldFit Diet! Start your fitness journey today.',
      status: 'Sent',
      frequency: 'One-time',
      createdBy: 'Admin',
    ...
    },
    {
      id: 2,
      type: 'Reminder',
      message: 'Don\'t forget to log your meals today and stay on track!',
      status: 'Scheduled',
      frequency: 'Daily',
      createdBy: 'System',
    ...
    },
    {
      id: 3,
      type: 'Promotion',
      message: 'Limited time offer: Upgrade to Premium and get 25% off!',
      status: 'Draft',
      frequency: 'One-time',
      createdBy: 'Admin',
    ...
    },
    {
      id: 4,
      type: 'Update',
      message: 'New workout plans available in the app. Check them out!',
      status: 'Sent',
      frequency: 'One-time',
      createdBy: 'Admin',
    ...
    },
    {
      id: 5,
      type: 'Reminder',
      message: 'Weekly progress summary is ready. See how you\'re doing!',
      status: 'Scheduled',
      frequency: 'Weekly',
      createdBy: 'System',
  ...
  // Update the handleCreateNotification function 
  const handleCreateNotification = () => {
    // Initialize with a new notification template
  ...
  const handleSaveNotification = (updatedNotification) => {
    if (isCreateModalOpen) {
      ...
      const newNotification = {
        ...
        createdBy: 'Admin', // Assuming current user is admin
        createdAt: new Date().toISOString().split('T')[0]
      };
      
      // If status is sent, add sentDate
      if (newNotification.status === 'Sent') {
        newNotification.sentDate = new Date().toISOString().split('T')[0];
        newNotification.reads = 0;
      }
      
      setNotifications([...notifications, newNotification]);
    } else {
      // Updating an existing notification
      const wasScheduled = selectedNotification.status === 'Scheduled';
      const nowSent = updatedNotification.status === 'Sent';
      
      // If changing from scheduled to sent, add sentDate
      if (wasScheduled && nowSent) {
        updatedNotification.sentDate = new Date().toISOString().split('T')[0];
        updatedNotification.reads = 0;
      }
  ...
  const calculateNextSendDate = () => {
    ...
    if (automationFrequency === 'Daily') {
      ...
      if (nextDays.length > 0) {
        // There's a selected day later this week
        nextDate.setDate(now.getDate() + (nextDays[0] - currentDay));
      } else {
        // No days later this week, go to first selected day next week
      ...
      if (monthlyDay <= monthLastDay) {
        // The day exists in the current month
        if (monthlyDay > currentDay) {
          // It's later this month
          nextDate.setDate(monthlyDay);
        } else {
          // It's already passed, go to next month
          nextDate.setMonth(now.getMonth() + 1);
          nextDate.setDate(monthlyDay);
        }
      } else {
        // The day doesn't exist in the current month (e.g., 31st in February)
  ...
  return (
    <div className="admin-page">
          ...
          <div className="modal notification-send-modal">
            ...
            <div className="notification-tabs">
              <button 
                ...
                onClick={() => {
                  // Add animation class removal and reflow trick
                  ...
                  // Add animation class after state update
                  setTimeout(() => {
                    const formContent = document.querySelector('.notification-send-modal form');
        ...
        <div className="modal-overlay">
          <div className="modal schedule-automation-modal">
            ...
            <div className="modal-footer">
              ...
              <button 
                onClick={() => {
                  // Create a new automated notification from the settings
                  const newAutomation = {
                    id: notifications.length + 1,
      ...
      {/* Create/Edit modals would be implemented similar to the view modal */}
