      ...
      if (selectedUser) {
        try {
          await import("../../api").then((api) =>
            api.deleteUser(selectedUser.id, token)
          );
          alert(
  ...
  const handlePromoteConfirm = async () => {
    ...
    setTimeout(async () => {
      ...
      if (selectedUser) {
        try {
          await import("../../api").then((api) =>
            api.promoteUser(selectedUser.id, token)
          );
          alert(
            `${selectedUser.firstName} ${selectedUser.lastName} has been promoted to admin.`
          );
        } catch (error) {
    ...
    <div className="admin-page">
      ...
      <div className="dashboard-bottom-section">
        <div className="users-card">
          <div className="users-card-header">
            ...
            <span className="users-card-count">
              {recentPremiumUsers.length} new
            </span>
          </div>
      ...
      {isDeleteConfirmOpen && selectedUser && (
        <div className={`modal-overlay ${deleteModalLeavingClass}`}>
          <div
            className={`modal confirmation-modal ${deleteModalLeavingClass}`}
          >
      ...
      {isPromoteConfirmOpen && selectedUser && (
        <div className={`modal-overlay ${promoteModalLeavingClass}`}>
          <div
            className={`modal confirmation-modal ${promoteModalLeavingClass}`}
          >
            <h2>Confirm Promotion</h2>
            <p>
              Are you sure you want to promote this user to an admin?
              <br />
              <strong className="promote-emphasis">
                {selectedUser.firstName} {selectedUser.lastName} (
                {selectedUser.email})
              </strong>
              <br />
              They will have access to the admin dashboard and permissions
